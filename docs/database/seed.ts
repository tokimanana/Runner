...
async function main() {
  ...
  const marketUK = await prisma.market.create({
    data: { code: 'UK', name: 'United Kingdom', tourOperatorId: tourOperator.id },
  });
    ...
    data: [
      ...
      },
      {
        ...
        ratesPerAge: JSON.stringify({
          ...
          [ageCatChildParis.id]: { rate: 0, order: 1, label: '1er enfant gratuit' }
        }),
        totalRate: 180,
        roomPriceId: suiteHighPrice.id,
      },
      {
        ...
        ratesPerAge: JSON.stringify({
          ...
          [ageCatChildParis.id]: { rate: 40, order: 2, label: '2ème enfant payant' }
        }),
        totalRate: 220,
        roomPriceId: suiteHighPrice.id,
      },
